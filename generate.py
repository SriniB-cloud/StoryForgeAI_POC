...
def read_story(story_id):
    ...
    with open(path) as f:
        content = f.read()
    console.print(Panel(
        f'[dim]{content.strip()}[/dim]',
        title=f'[cyan]Input — stories/{story_id}.txt[/cyan]',
        border_style='cyan'
    ))
    return content
...
def run_stage1(story_id, story_text):
    ...
    console.print('[dim]LIVE — Mistral 7B reads story file and extracts ParsedSpec[/dim]\n')
...
Return only the JSON:"""
    ...
    ac_t = Table(
        title=f'Acceptance Criteria ({len(parsed_spec["acceptance_criteria"])})',
...
def planner_agent(parsed_spec):
    console.print(Panel(
        '[bold]Agent 1 — Planner[/bold]\nReading config · deciding pyramid split · assigning test levels',
        border_style='blue'
    ))
    time.sleep(0.3)
    plan = {
...
def _build_fallback_tests(parsed_spec, api_target, ui_target, e2e_target):
    """
    Smart fallback — builds test cases directly from ParsedSpec AC.
    Works for ANY story — not hardcoded to US-001.
    Called only if Mistral fails 3 consecutive times in generator_agent.
    """
    console.print('[yellow]Using smart fallback — building test cases from AC[/yellow]')
    ac_list   = parsed_spec['acceptance_criteria']
    api_tests = []
    ui_tests  = []
    e2e_tests = []
    levels    = ['positive', 'negative', 'edge']

    # Build API tests from AC — cycle through positive/negative/edge
    for i, ac in enumerate(ac_list[:api_target]):
        level = levels[i % 3]
        api_tests.append({
            'id':                  f'TC-A{i+1:02d}',
            'area':                ac['id'],
            'test_level':          level,
            'priority':            'P0' if level == 'positive' else 'P1',
            'title':               f'[{level}] {ac["text"][:60]}',
            'acceptance_criteria': ac['id'],
            'expected_result':     f'{ac["id"]} behaviour verified'
        })

    # Build UI tests from AC
    for i, ac in enumerate(ac_list[:ui_target]):
        level = levels[i % 2]
        ui_tests.append({
            'id':                  f'TC-U{i+1:02d}',
            'area':                'UI',
            ...
            'title':               f'[{level}] UI — {ac["text"][:55]}',
            'acceptance_criteria': ac['id'],
            'expected_result':     f'UI {ac["id"]} verified'
        })

    # Build E2E tests — always 3 standard journeys
    all_ac_ids = ', '.join([a['id'] for a in ac_list])
    e2e_scenarios = [
        ('positive', f'Full happy path journey — {parsed_spec["action"][:50]}', 'P0'),
        ('negative', 'Journey with invalid input — appropriate error shown',     'P0'),
        ('edge',     'Edge case journey — session or boundary condition',        'P1'),
    ]
    for i, (level, title, priority) in enumerate(e2e_scenarios[:e2e_target]):
        e2e_tests.append({
            'id':                  f'TC-E{i+1:02d}',
            'area':                'Journey',
            'test_level':          level,
            'priority':            priority,
            'title':               title,
            'acceptance_criteria': all_ac_ids,
            'expected_result':     f'Journey {level} scenario verified'
        })

    console.print(f'  Fallback: [blue]{len(api_tests)} API[/blue] · [green]{len(ui_tests)} UI[/green] · [yellow]{len(e2e_tests)} E2E[/yellow]')
    console.print('[green]✅ Smart fallback complete[/green]\n')
    return api_tests, ui_tests, e2e_tests


def generator_agent(parsed_spec, plan):
    console.print(Panel(
        '[bold]Agent 2 — Generator[/bold]\nCalling Mistral · generating test cases per AC · positive · negative · edge',
        border_style='blue'
    ))
    ...
    e2e_target = max(2, round(ac_count * plan['e2e_pct'] / 100))
...
Return only the JSON object:"""
    ...
    for attempt in range(3):
        try:
            ...
            console.print(f'[yellow]Attempt {attempt+1} schema error: {e}[/yellow]')
            if attempt == 2:
                # Smart fallback — works for any story, not hardcoded
                return _build_fallback_tests(parsed_spec, api_target, ui_target, e2e_target)


def critic_agent(api_tests, ui_tests, e2e_tests, parsed_spec):
    console.print(Panel(
        '[bold]Agent 3 — Critic[/bold]\nChecking coverage gaps · validating all ACs covered',
        border_style='blue'
    ))
    time.sleep(0.3)
...
def refiner_agent(api_tests, ui_tests, e2e_tests, missing_acs, parsed_spec):
    console.print(Panel(
        '[bold]Agent 4 — Refiner[/bold]\nDeduplicating · fixing missing ACs · locking TestSpec',
        border_style='blue'
    ))
    time.sleep(0.3)
    if missing_acs:
        ...
        for i, ac_id in enumerate(missing_acs):
            ac_text = next(
                (a['text'] for a in parsed_spec['acceptance_criteria'] if a['id'] == ac_id),
                ac_id
            )
            api_tests.append({
                'id':                  f'TC-A{90+i:02d}',
                ...
                'test_level':          'positive',
                'priority':            'P1',
                'title':               f'Verify: {ac_text[:55]}',
                'acceptance_criteria': ac_id,
                'expected_result':     f'{ac_id} verified'
            })
...
def run_stage2(story_id, parsed_spec):
    ...
    console.print('[dim]LIVE — Mistral 7B generates test cases via 4-agent pipeline[/dim]\n')
...
def run_stage4(all_tests, api_tests, ui_tests, e2e_tests, parsed_spec):
    ...
    for ac in parsed_spec['acceptance_criteria']:
        cov.add_row(
            ac['id'], ac['text'],
            '[green]✅[/green]' if ac['id'] in covered_acs else '[red]❌[/red]'
        )
    console.print(cov)
    ...
    pyr.add_row('API', 'REST-assured + Java 17',  str(len(api_tests)),
                f"{round(len(api_tests)/total*100)}%",  f"~{cfg['pyramid']['api']['target_pct']}% ✅")
    pyr.add_row('UI',  'Playwright + TypeScript', str(len(ui_tests)),
                f"{round(len(ui_tests)/total*100)}%",   f"~{cfg['pyramid']['ui']['target_pct']}% ✅")
    pyr.add_row('E2E', 'Playwright + TypeScript', str(len(e2e_tests)),
                f"{round(len(e2e_tests)/total*100)}%",  f"~{cfg['pyramid']['e2e']['target_pct']}% ✅")
    console.print(pyr)

    vc = 'green' if verdict == 'PASS' else 'red'
    console.print(Panel(
        f'Coverage : [bold]{len(covered_acs)}/{len(ac_ids)} = {coverage_pct}%[/bold]  |  '
...
def run_stage5(story_id, parsed_spec, test_data, api_tests, ui_tests, e2e_tests):
    ...
    console.print(out_t)
    console.print(Panel(
        f'[green]✅ {total_lines} lines generated · written to /generated/{story_id}/ · no LLM · 100% deterministic[/green]',
        border_style='green'
    ))
...
def run_pipeline(story_id):
    console.print(Panel.fit(
        ...
        f'[dim]Mode   : LIVE — Mistral 7B via Ollama[/dim]',
        border_style='green'
    ))
    console.print()
    ...
    for k, v in [
        ...
        ('Data Leakage',     '[green]Zero — all processing on-premise[/green]'),
        ('LLM',              'Mistral 7B via Ollama (live)'),
    ]:
        sm.add_row(k, v)
    console.print(sm)
    console.print(Panel(
...
def run_batch():
    ...
    console.print(Panel(
        f'[bold cyan]Batch Runner — {len(stories)} stories[/bold cyan]\n'
        f'[dim]{" · ".join(stories)}[/dim]',
        border_style='cyan'
    ))
    results = []
    ...
    for r in results:
        ...
        gl += r['lines']
        bt.add_row(
            r['story_id'], str(r['api']), str(r['ui']), str(r['e2e']),
    ...
    console.print(bt)
    console.print(Panel(
        f'[green]✅ {len(results)} stories · {gt} test cases · {gl} lines[/green]\n'
        f'[dim]Output: /generated/{{story_id}}/[/dim]',
        border_style='green'
    ))
